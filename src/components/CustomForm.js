...
const CustomForm = (props)=>{
    ...
    const [buttonText, setButtonText] = useState('Complete fields to enable submission');
    ...
    return(
        ...
        {props.inputsArray.map((input, index)=>{
            ...
            return (
                ...
                <label htmlFor={input.name} className={input.valid!=1 ? "m-2 w-1/4" : "m-2 w-1/4 text-red-500"}>{input.valid!=1 ? input.label : input.invalidMsg} </label>
                
                <input type={input.type} onBlur={processValidation} name={input.name} onChange={updateReturnObj} className={input.valid!=1 ? 'm-5 w-1/2 border-solid border-2 border-black rounded-md p-2' : ' w-1/2 m-5 border-dotted border-2 rounded-md p-2 bg-red-200'}></input>
              
                </div>
            )
        })}
        <button onClick={submitHandler} className="disabled:opacity-50 w-full font-bold bg-red-500 text-xl m-auto text-white hover:bg-red-200 hover:text-black rounded-lg">{buttonText}</button>
        </form>
